...
**Current Version:** v2.3 (v2.2 + Operational Learning Infrastructure + Deadline Confidence + Living Topic-State, 44 MCP tools)
**Status:** v2.3 deployed on production revision `gianluigi-00067-rtf` (`/health` returning 200, `gcloud run deploy --source .`). Shipped as 5 PRs in one session: PR 1 (DB migration — `approval_observations` table + `tasks.deadline_confidence` + `topic_threads.state_json` + expression index), PR 2 (deadline confidence tiers — EXPLICIT/INFERRED/NONE classification at extraction, reminder + proactive-alert filtering to EXPLICIT only, `~` prefix on INFERRED in morning brief, 18 legacy tasks backfilled to INFERRED), PR 3 (observation capture layer — `log_approval_observation()` hooked into meeting/gantt/signal/prep approve+reject+edit, debrief quick_inject, sheets_sync; new `get_approval_stats` MCP tool), PR 5 (inline deadline buttons — Clear-date button added, `+1 Week` now stamps EXPLICIT + logs observation, voice-aligned confirmations), PR 4 (living topic-state summaries — new `TopicState` schema with `current_status`/`stakeholders`/`open_items`/`last_decision`/`key_facts`, `update_topic_state()` Haiku-powered incremental update post-approval, MCP `get_topic_thread(include_state=True)` exposure, morning brief surfaces blocked/stale topics with 3-item cap, QA scheduler daily staleness sweep, 2 canonical projects seeded as new threads, 55/74 existing threads backfilled via Sonnet one-time pass). 2006 tests passing, 21 pre-existing failures baselined (23 total - 2 fixed by PR 2 fixture updates). Prior status: Telegram Comms Overhaul on `gianluigi-00066-tf8`, Sheets-Sync Hardening on `gianluigi-00065-8tj`, Tier 3 + Live Ops on `gianluigi-00064-9ps`.
