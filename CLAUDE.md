...
**Current Version:** v2.3.1 (v2.3 + post-deploy stabilization pass, 44 MCP tools)
**Status:** v2.3.1 deployed on production revision `gianluigi-00071-8hm` (`/health` returning 200, `gcloud run deploy --source .`). Shipped as 3 focused PRs after the 2026-04-14 first-production-run review of v2.3 (Franciacorta Status Meeting): PR 1 — broken-pipe retry on Drive + Gmail watchers (`services/google_drive.py` + `services/gmail.py` + `schedulers/personal_email_scanner.py` got `_execute_with_retry` mirroring the proven Sheets pattern; nulls `_service` between attempts to force httplib2-transport rebuild on Cloud Run idle-then-wake stale sockets; download paths rewritten to drop the inline `@retry` decorator in favor of the same rebuild-aware loop; resolves 3 critical_error events/day in audit_log), PR 2 — `deadline_confidence` extraction prompt fix (`processors/transcript_processor.py` JSON schema example now shows the bare value `"EXPLICIT"` instead of the wordy pipe-separated `"EXPLICIT | INFERRED | NONE — see EXTRACTION INSTRUCTIONS"`; new dedicated `DEADLINE_CONFIDENCE` rule in ACTION ITEM EXTRACTION RULES; post-extraction safety net normalizes `tasks_to_store` so non-null deadline + NONE/missing confidence → EXPLICIT and null deadline + non-NONE → NONE), PR 3 — label-preservation guards (`create_decisions_batch` always writes label even when empty string instead of skipping via truthiness; diagnostic warnings in `deduplicate_tasks` and `create_tasks_batch` surface label-coverage regressions in logs with meeting_id for grep-ability; 2 regression-guard tests). Test count up to 2014 (8 new), same 21 pre-existing failures baselined. Prior status: v2.3 on `gianluigi-00068-h4j` (topic_state max_tokens fix), then `gianluigi-00067-rtf` (initial v2.3 deploy), Telegram Comms Overhaul on `gianluigi-00066-tf8`, Sheets-Sync Hardening on `gianluigi-00065-8tj`.

**v2.3.1 stabilization pass (2026-04-14):** First production run of v2.3 surfaced 3 issues — none architectural, all contained. (1) Drive + Gmail watchers had been silently dying with `[Errno 32] Broken pipe` on every tick (Drive 15min, Gmail 2h) for days; the Sheets retry pattern from earlier work hadn't been applied to the other Google service wrappers. (2) Opus was dropping `deadline_confidence` on extracted tasks that had explicit dates, landing them as NONE in DB and silently suppressing all reminders — root cause was a wordy JSON schema example string the model misread as a description pointer rather than a value. (3) `tasks.label` and `decisions.label` columns landed NULL on the Franciacorta meeting despite topic_threading creating 9 distinct topic mentions in the same dicts — partial root cause was `create_decisions_batch` skipping empty-string labels via truthiness check; full root cause not yet observed (now instrumented). Rejected meetings cleanup (`scripts/cleanup_rejected_meetings.py`) verified clean — 3 tombstones, 0 orphan children, expected post-T1.9.
