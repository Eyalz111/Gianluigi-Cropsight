...
Gianluigi — CropSight's AI operations assistant ("AI Office Manager") for a 4-person AgTech founding team. Ingests meeting transcripts (Tactiq → Drive), extracts tasks/decisions/open questions, threads them across meetings, and serves the CEO (Eyal) via Telegram and Claude.ai MCP (46 tools).
...
6. `services/mcp_server.py` — MCP server with 46 tools (read + write)
