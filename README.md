...
- **Document ingestion:** Versioning with content hash dedup, CropSight-specific classification, Dropbox sync (when enabled)
- **Daily QA agent:** Automated extraction quality, distribution completeness, scheduler health, and data integrity checks
...
Tactiq (transcript) --> Google Drive --> Gianluigi (Cloud Run)
                   ...
                   Continuity AI         Embeddings            Gantt Chart
                        |                     |                     |
                        +---------------------+---------------------+
                                              |
                              +---------------+---------------+
...
|----------|-------|---------|
| SYSTEM (7) | get_system_context, get_full_status, get_system_health, get_cost_summary, get_pending_approvals, get_upcoming_meetings, run_qa_check | Operational state + QA |
...
- **Sensitivity follows data.** Tags applied at ingestion, LLM-classified, propagated to all child items.
...
core/            # LLM interface, system prompt, retry logic, cost calculator, health monitor
...
| `V1_DESIGN.md` | v1.0 design specification (Phases 0-10) |
