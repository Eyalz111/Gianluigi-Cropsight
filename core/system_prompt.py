...
# Forward-facing rich template (PR7, SUMMARY_RICH_ENABLED). A SEPARATE registry
# key (summary_template_rich) so the legacy template above is never touched.
# The optional blocks ({tl_dr}, {decision_intelligence}, {area_rollup},
# {risks}, {changed_since}) each carry their own leading newline + header and
# collapse to "" when empty, so absent sections simply don't render. The Action
# Items table gains Area + Urgency columns.
SUMMARY_TEMPLATE_RICH = """# Meeting Summary: {title}
**Date:** {date} | **Duration:** {duration} minutes
**Participants:** {participants}
**Sensitivity:** {sensitivity}{topic_context}{tl_dr}
...
## Action Items
| # | Task | Area | Owner | Deadline | Priority | Urgency | Ref |
|---|------|------|-------|----------|----------|---------|-----|
{tasks}{area_rollup}{risks}

## Open Questions & Risks
{open_questions}{changed_since}
...
def format_summary(
    ...
    `rich` (PR7, SUMMARY_RICH_ENABLED) switches to the forward-facing
    summary_template_rich: Action Items gain Area + Urgency columns and the
    caller-supplied blocks (`tl_dr`, `decision_intelligence`, `area_rollup`,
    `risks_text`, `changed_since`) render as sections. Each block is a complete,
    tier-safe string built upstream — this function still only renders what it
    is handed. `rich=False` ⇒ byte-identical legacy output.
    ...
    # Format tasks. Rich mode adds Area + Urgency columns (the operational
    # floor); legacy mode is byte-for-byte the original 6-column row.
    tasks_text = ""
...
"""
    ...
    if rich:
        template = prompt_registry.get("summary_template_rich") or SUMMARY_TEMPLATE_RICH
        return template.format(
            title=meeting_title,
            date=meeting_date,
