...
- **Urgency + Category (realigned 2026-06):** tasks carry `urgency` (H/M/L = time-pressure, SEPARATE from `priority` = importance) and `category` — the task taxonomy IS the Gantt board's area list: `PRODUCT & TECHNOLOGY`, `SALES & BUSINESS DEVELOPMENT`, `FUNDRAISING & INVESTOR RELATIONS`, `LEGAL, CORPORATE & FINANCE`, `CLIENT DELIVERY & OPERATIONS`, `TEAM & HUMAN RESOURCES`, or `General` for a genuine misfit. (The old separate `area` field is gone; the old category names like "BD & Sales" are retired.) Both `create_task()` and `update_task()` accept `urgency=` and `category=`. **No invented dates:** for an "ASAP / urgent, no date given" task set `urgency="H"` and leave the deadline empty — never fabricate a date. Use `priority` for "how important", `urgency` for "how time-critical".
- **Archiving (2026-06):** `update_task(task_id, status="archived")` removes a task from the working view (Tasks sheet, briefs, digests) while keeping it in the DB for history. Use when Eyal says a task is irrelevant/duplicate/stale — NOT when it was completed (that's `done`). Archived rows land on the sheet's Archive tab.
...
| Tool | Purpose |
|------|---------|
| `get_tasks(assignee?, status?, category?)` | Query tasks with filters. Returned rows include `deadline_confidence` (EXPLICIT / INFERRED / NONE), `urgency` (H/M/L), and `category` (a Gantt board area or `General`) |
| `create_task(title, assignee?, deadline?, label?, urgency?, category?)` | Create new task (confirm first). `urgency`=H/M/L time-pressure (H for ASAP, no fake date); `category`=a Gantt board area name or `General`. Pass `deadline_confidence="EXPLICIT"` when Eyal commits to a specific date |
| `update_task(task_id, status?, deadline?, priority?, assignee?, urgency?, category?)` | Update existing task (confirm first). Can set `urgency`/`category`; `status="archived"` removes it from the working view (kept in DB). Changes also sync to the Tasks sheet. Pass `deadline_confidence="EXPLICIT"` when changing the deadline to a chosen date |
