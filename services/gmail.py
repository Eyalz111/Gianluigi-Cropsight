...
# --- Email-thread ingestion helpers [2026-07-25] -----------------------------
# The quote chain is where the "long thread" cost + duplication lives: a 40-reply
# thread re-sends all 40 in every message's quoted history. We CUT it before
# extraction (only the new prose is extracted), and keep the CUT PART available
# for gap recovery (backfilling messages we were never sent — see email_ingest).
# Deliberately regex, not the Talon library: Talon's ML signature model pulls
...
def _quote_cut_index(text: str) -> int:
    """Earliest index at which the quoted history begins (len(text) if none)."""
    cut = len(text)
    for p in _QUOTE_CUT_PATTERNS:
...
def strip_quoted_text(body: str) -> str:
    """Return only the NEW prose of an email — quoted history + trailing signature
    removed. Empty string if the message is nothing but a quote."""
    if not body:
        return ""
    text = body.replace("\r\n", "\n")
    new = text[:_quote_cut_index(text)]
    new = re.split(r"\n-- \n", new)[0]          # drop a trailing "-- \n" signature block
    return new.strip()


def extract_quoted_text(body: str) -> str:
    """The complement of strip_quoted_text — the quoted history block (for gap
    recovery when we were never sent an intervening message)."""
    if not body:
        return ""
    text = body.replace("\r\n", "\n")
    return text[_quote_cut_index(text):].strip()
...
class GmailService:
    ...
    def _execute_with_retry(self, request_factory, max_retries: int = 3, base_delay: float = 1.0):
        ...
        the next request_factory() call builds a fresh service. Cloud Run
        idle-then-wake cycles leave stale sockets in httplib2's pool.

        Caller passes a ZERO-ARG callable that constructs the request so
        we can rebuild against the fresh service.
        """
        import time
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Gmail API retry {attempt + 1}/{max_retries}: "
                ...
                if any(k in error_str for k in (
                    ...
                    if attempt < max_retries - 1:
                        delay = base_delay * (2 ** attempt)
                        logger.warning(
                            f"Gmail API transient error retry {attempt + 1}/{max_retries}: "
                            f"{type(e).__name__}: {e}. Rebuilding service, retrying in {delay:.1f}s..."
                        )
                        self._service = None
                        time.sleep(delay)
    ...
    async def send_meeting_summary(
        ...
        # Build brief plain-text body (intro only — full content in the attachment)
        tldr_line = f"\nTLDR: {executive_summary}\n" if executive_summary else ""
        task_count = len(tasks) if tasks else 0

        # PDF replaces the Word doc as the attachment (2026-07-12). Fall back to
        # the .docx if no PDF was produced, so the email always carries the summary.
        attach_data = pdf_bytes or docx_bytes
        attach_is_pdf = bool(pdf_bytes)
        attach_label = "PDF" if attach_is_pdf else "Word document"
...
{task_count} action items captured. Full details in the attached {attach_label}.
...
<p><strong>{task_count} action items captured.</strong> Full details in the attached {attach_label}.</p>
...
"""
        ...
        # Build email with the summary attachment (PDF preferred, .docx fallback).
        if attach_data:
            from email.mime.base import MIMEBase
            from email import encoders
            ...
            # Send via the retry-wrapped sender (bypass send_email helper for
            # attachment support, but keep the broken-pipe retry) so an
            # idle-wake stale socket is retried instead of silently dropping
            # the approved team-summary email. [audit P3-04]
            try:
    ...
    async def send_approval_request(
        ...
        if tasks:
            ...
            # off-path stays the original 4-column grey-header table.
            ...
            for t in tasks:
                ...
                assignee = _html_escape(t.get("assignee", "team") or "team")
                deadline = _html_escape(str(t.get("deadline") or "—"))
                if branded:
                    area = _html_escape(t.get("category", "") or "General")
                    urgency = (t.get("urgency") or "M").upper()
                    ucolor = {"H": "#C0392B", "M": "#C9A227", "L": "#6B6B6B"}.get(urgency, "#C9A227")
                    rows += (
                        f"<tr>"
                        f"<td style='{td}'>[{priority}]</td>"
                        f"<td style='{td}'>{title}</td>"
                        f"<td style='{td}'>{area}</td>"
                        f"<td style='{td}'>{assignee}</td>"
                        f"<td style='{td}'>{deadline}</td>"
                        f"<td style='{td}color:{ucolor};font-weight:bold;'>{urgency}</td>"
                        f"</tr>"
                    )
                else:
                    rows += (
            ...
            if branded:
                th = "padding:4px 8px;border:1px solid #ddd;text-align:left;color:#fff;"
                header = (
                    f"<tr style='background:#1A7A4C;'>"
                    f"<th style='{th}'>Pri</th><th style='{th}'>Task</th>"
                    f"<th style='{th}'>Category</th><th style='{th}'>Assignee</th>"
                    f"<th style='{th}'>Deadline</th><th style='{th}'>Urgency</th></tr>"
                )
            else:
                th = "padding:4px 8px;border:1px solid #ddd;text-align:left;"
                header = (
                    f"<tr style='background:#f5f5f5;'>"
                    f"<th style='{th}'>Pri</th><th style='{th}'>Task</th>"
                    f"<th style='{th}'>Assignee</th><th style='{th}'>Deadline</th></tr>"
                )
            html_parts.append(
...
"""
    ...
    async def get_message(self, message_id: str) -> dict | None:
        ...
        try:
            ...
            # Case-insensitive view — header casing varies by client, and the
            # threading headers (References / In-Reply-To) must resolve reliably.
            hlow = {h["name"].lower(): h["value"] for h in headers}
            ...
            return {
                ...
                "message_id": hlow.get("message-id", header_dict.get("Message-ID", "")),
                "references": hlow.get("references", ""),
                "in_reply_to": hlow.get("in-reply-to", ""),
                "body": body,
    ...
    async def get_thread(self, thread_id: str) -> list[dict]:
        """All messages of a Gmail thread that are in THIS mailbox, oldest first.

        Each: {id, message_id, from, to, cc, date, references, in_reply_to, body}.
        Only returns what the mailbox actually holds — messages Gianluigi was
        never sent (e.g. while off the CC) are absent; the References header +
        each message's quoted history recover those (email_ingest gap handling).
        """
        try:
            ...
            for m in thread.get("messages", []):
                ...
                out.append({
                    "id": m.get("id", ""),
                    "threadId": thread_id,
                    "message_id": hlow.get("message-id", ""),
                    "from": hlow.get("from", ""),
                    "to": hlow.get("to", ""),
                    "cc": hlow.get("cc", ""),
                    "date": hlow.get("date", ""),
    ...
    def _extract_body(self, payload: dict) -> str:
        """Extract the best text body from an email payload.

        Robustness [2026-07-27]:
        - Prefer text/plain; fall back to text/html (tags stripped) so an
          HTML-only email isn't ingested as an empty body.
        - Decode each part with its DECLARED charset (Content-Type charset=...)
          rather than a hard utf-8 — a Hebrew email sent as windows-1255 /
          iso-8859-8 would otherwise have its Hebrew silently dropped by
          decode(utf-8, errors="ignore").
        """
        plain = self._collect_part(payload, "text/plain")
        if plain:
            return plain
        html = self._collect_part(payload, "text/html")
        if html:
            return self._html_to_text(html)
        # Last resort: any top-level body data (legacy behavior).
        if payload.get("body", {}).get("data"):
            return self._decode_part(payload)
        return ""

    def _collect_part(self, payload: dict, want_mime: str) -> str:
        """Depth-first search for a part of `want_mime`; return its decoded text."""
        mime = (payload.get("mimeType") or "").lower()
        if mime == want_mime and payload.get("body", {}).get("data"):
            return self._decode_part(payload)
        for part in payload.get("parts", []) or []:
            found = self._collect_part(part, want_mime)
            if found:
                return found
        return ""

    def _decode_part(self, part: dict) -> str:
        """Decode a part's base64 body using its declared charset (fallback utf-8)."""
        data = part.get("body", {}).get("data")
        if not data:
            return ""
        raw = base64.urlsafe_b64decode(data)
        charset = self._part_charset(part) or "utf-8"
        try:
    ...
    @staticmethod
    def _part_charset(part: dict) -> str | None:
    ...
    def _html_to_text(html: str) -> str:
        """Minimal HTML->text: drop script/style, keep line breaks, strip tags,
        unescape entities. Good enough to feed extraction; not a full renderer."""
